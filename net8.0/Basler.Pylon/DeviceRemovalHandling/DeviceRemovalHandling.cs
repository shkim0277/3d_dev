                ...
                using (Camera camera = new Camera())
                {
                    ...
                    while (camera.StreamGrabber.IsGrabbing && stopWatch.ElapsedMilliseconds < cTimeOutMs)
                    {
                        try
                        {
                            ...
                            using (IGrabResult grabResult = camera.StreamGrabber.RetrieveResult(10000, TimeoutHandling.ThrowException))
                            {
                                ...
                                if (grabResult.GrabSucceeded)
                                {
                                    ...
                                    {
                                        Console.WriteLine("Grabbed an image. SizeX: {0} SizeY: {1}", grabResult.Width, grabResult.Height);
                                    }
                                }
                            }
                        }
