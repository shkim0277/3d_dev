                ...
                using (Camera camera = new Camera(DeviceType.GigE, CameraSelectionStrategy.FirstFound))
                {
                    ...
                    switch (keyPressed.KeyChar)
                    {
                        ...
                        case 'M':
                            ...
                            camera.Parameters[PLStream.TransmissionType].TrySetValue(PLStream.TransmissionType.UseCameraConfig);
                            ...
                            if ((camera.Parameters[PLStream.DestinationAddr].GetValue() != "0.0.0.0") &&
                                 (camera.Parameters[PLStream.DestinationPort].GetValue() != 0))
                            {
                        ...
                        case 'C':
                            ...
                            // In this case, the IP Address and the IP port must also be set.
                            camera.Parameters[PLStream.TransmissionType].SetValue(PLStream.TransmissionType.Multicast);
                            //camera.Parameters[PLGigEStream.DestinationAddr].SetValue("239.0.0.1");
                            //camera.Parameters[PLGigEStream.DestinationPort].SetValue(49152);

                            // Maximize the image area of interest (Image AOI).
                            camera.Parameters[PLCamera.OffsetX].SetValue(camera.Parameters[PLCamera.OffsetX].GetMinimum());
                            camera.Parameters[PLCamera.OffsetY].SetValue(camera.Parameters[PLCamera.OffsetY].GetMinimum());
