        ...
        public bool Configure()
        {
            ...
            if (base.Open(1000, TimeoutHandling.Return))
            {
                ...
                if (this.GetSfncVersion() < Sfnc2_0_0)
                {
                    ...
                    exposureEndDataName = PLCamera.ExposureEndEventTimestamp;
                    exposureEndFrameID = PLCamera.ExposureEndEventFrameID;
                    exposureEndTimestamp = PLCamera.ExposureEndEventTimestamp;
                }
                else // For SFNC 2.0 cameras, e.g. USB3 Vision cameras
                {
                    exposureEndDataName = PLCamera.EventExposureEnd;
                    exposureEndFrameID = PLCamera.EventExposureEndFrameID;
                    exposureEndTimestamp = PLCamera.EventExposureEndTimestamp;
                }
            ...
            try
            {
                ...
                using (EventCamera eventCamera = new EventCamera())
                {
                    ...
                    while (eventCamera.StreamGrabber.IsGrabbing)
                    {
                        ...
                        using (IGrabResult grabResult = eventCamera.StreamGrabber.RetrieveResult(5000, TimeoutHandling.ThrowException))
                        {
